...
# Endpoint cho trang chủ Web Demo UI
@app.get("/", response_class=FileResponse)
def read_index():
    return FileResponse("index.html")

# Endpoint API giữ nguyên
...
def get_recommendation(customer_id: str, use_history: bool = True):
    ...
    cursor.execute("SELECT actual_items FROM ground_truth WHERE customer_id = ?", (customer_id,))
    ...
    for i_id in rec_item_ids:
        info = item_info_map.get(
            i_id,
            {"item_id": i_id, "brand": "N/A", "category_l1": "N/A", "category_l2": "N/A", "price": 0.0},
        ).copy()
